...
void  HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	...
	if(htim -> Instance == TIM1){
		...
		if (send == 1){
			odczyt_x = SPIRead(Out_X);
			odczyt_y = SPIRead(Out_Y);
			odczyt_z = SPIRead(Out_Z);
			size = sprintf(Data, "Os X: %d,  Os Y: %d,  Os Z: %d\n", odczyt_x, odczyt_y, odczyt_z);
